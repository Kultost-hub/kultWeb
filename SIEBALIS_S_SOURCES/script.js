(function () {
  ...
  const countDown = new Date("May 31, 2026 23:59:59").getTime(),
...
let endscreenContent = {win: 'Ты выиграл.', loose: 'Ты проиграл.'};
...
const checkVictory = () => {
	...
	if (win) {
		...
		if (wins === 6) {
			setTimeout(Anekdot, 400);
		}
		if (wins === 8) {
			setTimeout(Alex, 400);
		}
		if (wins === 13) {
			setTimeout(Dastr, 400);
		}
	}
